...
from app.utils import PerfLog, parse_rgb_str, with_retry, with_semaphore
...
async def download_preview(
    ...
    async def fetch_tile(x: int, y: int) -> None:
        ...
        tiles[(x, y)] = resp.raise_for_status().read()
    ...
    logger.info(f"Downloaded <g>{len(tiles)}</> tiles (<y>{perf.elapsed:.2f}</>s)")
...
async def calc_template_diff(
    ...
    *,
    ...
    template_bytes = cfg.load_im().save(buffer := io.BytesIO(), format="PNG") or buffer.getvalue()
